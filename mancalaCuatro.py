...
class CooperativeQLearningAgent(QLearningAgent):
    def __init__(self):
        super().__init__()

    def synchronize_q_table(self, other_agent):
        for state_key in other_agent.q_table:
            if state_key not in self.q_table:
                self.q_table[state_key] = np.zeros(6)
            self.q_table[state_key] = (self.q_table[state_key] + other_agent.q_table[state_key]) / 2
...
def play_game(agent1, agent2, game):
    ...
    while not game.is_game_over():  # Continuar hasta que se acaben las fichas de un lado
        ...
        # Sincronizar las tablas Q para cooperación
        agent1.synchronize_q_table(agent2)
        agent2.synchronize_q_table(agent1)
...
def run_multiple_games(num_games):
    rewards = []
    agent1 = CooperativeQLearningAgent()
    agent2 = CooperativeQLearningAgent()
