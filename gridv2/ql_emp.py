...
class SimpleEnv(MiniGridEnv):
    ...
    def __init__(self, size=13, agent_start_pos=(6, 6), agent_start_dir=0, num_pasos = 6, max_steps: int | None = None, **kwargs):
    ...
    def calculate_empowerment_matrix(self):
        ...
        for x in range(1, self.grid_size - 1):  # Bordes de paredes
    ...
    def initialize_q_table(self):
        """ Función para inicializar la Q-table con los valores de empowerment correspondientes
            a cada posición y acción posible."""
        self.q_table = {}
        
        # Para cada posición en el grid (incluyendo los bordes)
            ...
            for col in range(0, self.grid.width):
                for quadrant_index in range(len(self.quadrants)):
                    ...
                    for action in range(4):
                        next_pos = self.next_position((col, row), action)
                        # Usar el valor de empowerment de la posición siguiente
                        # Si es una pared (0,0), asignar valor mínimo
                        emp_value = self.empowerment_grid[next_pos[1], next_pos[0]]
                        self.q_table[state][action] = emp_value

    def save_q_table(self, filename="q_table_empql.txt"):
    ...
    def get_state_value(self, state):
        ...
        if state not in self.q_table:
            self.q_table[state] = {}
            current_pos = (state // 100, state % 100)
            for action in range(4):
                next_pos = self.next_position(current_pos, action)
                emp_value = self.empowerment_grid[next_pos[1], next_pos[0]]
                self.q_table[state][action] = emp_value
        return self.q_table[state]

    def q_learning_agent(self, alpha=0.5, gamma=0.95, epsilon=0.9, min_epsilon=0.01, decay_rate=0.99, max_steps=70, episodes=700):
        """ Q-Learning que mantiene los valores de empowerment en la Q-table."""
        min_steps_to_goal = float('inf')
        best_episode = None
        successful_episodes = 0
        
        # Inicializar la Q-table con los valores de empowerment
        ...
        for episode in range(episodes):
            ...
            while steps < max_steps:
                ...
                if random.uniform(0, 1) < epsilon:
                    action = random.choice(list(state_values.keys()))
                else:
                ...
                reward = self.empowerment_grid[new_pos[1], new_pos[0]]
                ...
                next_emp = self.empowerment_grid[new_pos[1], new_pos[0]]
                self.q_table[state][action] = next_emp
                current_pos = new_pos
