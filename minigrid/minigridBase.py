...
class SimpleQEnv(MiniGridEnv):
    def __init__(self, size=10, agent_start_pos=(1, 1), agent_start_dir=0, max_steps=100, **kwargs):
...
def q_learning(env, num_episodes=1, alpha=0.1, gamma=0.99, epsilon=0.9, epsilon_decay=0.99):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            state_index = state_to_index(env.agent_pos)
            ...
            next_state_index = state_to_index(env.agent_pos)
            ...
            total_reward += reward
            pos = (int(env.agent_pos[0]), int(env.agent_pos[1]))
            step_count += 1

            print(f"Paso: {step_count}  Estado: {pos}, Acción: {action}, Recompensa: {reward}, Recompensas restantes: {env.remaining_rewards}")
...
def main():
    ...
    while not done:
        state_index = obs['agent_pos'][0] * env.width + obs['agent_pos'][1]
